...
export const ROLI_EMAIL_NOT_VERIFIED_ERROR_CODE = 404;
export const ROLI_SERVER_ERROR_CODE = 400;
...
export class CodeError extends Error {
    ...
    static getMessage(category: string, error: string) {
        switch (category) {
            case "Platform":
                switch (error) {
                    case "MaxServiceLimitReached":
                        return `The total service limit has been reached. See the ${SERVICE_NOTICES_CHANNEL} channel for when the limit will be increased.`
                    case "UserServiceLimitReached":
                        return "The maximum number of services already exist on your account. You must delete an existing service before you can boot a new one."
                    ...
                    case "FailedToCreateAccountInService":
            ...
            case "BusinessLogic":
                switch (error) {
                    case "DuplicateServiceName":
                        return "A service already exists with that name."
                    case "ServiceNotFoundByName":
                        return "A service by that name was not found."
                    case "UserNotFound":
            ...
            case "External":
                return "Roli internal error: " + error;
            case "CodeParse":
                return "Service compilation error: " + error;
        }
    }
}
...
export function parseError(response_obj: any): Error {
    ...
    if (!response_obj.hasOwnProperty('type'))
        throw new Error("Unknown error returned from Admin: type wasn't found");
    ...
    switch (response_obj.type) {
        ...
        default:
            throw new Error(`Unknown error type ${response_obj.type} returned from Admin`);
    }
}